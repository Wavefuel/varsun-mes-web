		...
		<div className="flex flex-col min-h-screen bg-background-dashboard">
			...
			<header className="sticky top-0 z-30 bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between shadow-sm">
				<div className="flex items-center gap-3">
					...
					<div>
						<span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block leading-none mb-0.5">
							Execution &rsaquo; Tagging
						</span>
				...
				>
					<div
						className={`size-5 bg-white rounded-full shadow-sm transition-transform ${tagMode ? "translate-x-[20px]" : "translate-x-0"}`}
					></div>
				</div>
			</header>
				...
				<div className="grid grid-cols-3 gap-2">
					...
					<div className="bg-white rounded-xl border border-gray-200 p-3 shadow-sm flex flex-col items-center justify-center text-center">
						...
						<div className="text-sm font-bold text-gray-800">
							45 <span className="text-[10px] text-gray-500 font-medium">min</span>
						</div>
					</div>
					...
					<div className="bg-white rounded-xl border border-gray-200 p-3 shadow-sm flex flex-col items-center justify-center text-center">
						...
						<div className="text-sm font-bold text-gray-800">
							15 <span className="text-[10px] text-gray-500 font-medium">min</span>
						</div>
					</div>
				</div>
				...
				<section>
					...
					<div className="space-y-4">
						{events.map((event) => (
							<div
								key={event.id}
								className={`bg-white rounded-xl border p-4 shadow-sm ${event.type === "untagged" ? "border-amber-200" : "border-red-100"}`}
							>
								{/* Event Header */}
								<div className="flex justify-between items-start mb-4">
									<div className="flex items-center gap-3">
										<div
											className={`size-10 rounded-lg flex items-center justify-center ${event.type === "untagged" ? "bg-amber-100 text-amber-600" : "bg-red-100 text-red-500"}`}
										>
								...
								<div className="space-y-3">
									...
									<div>
										<label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5 block ml-1">
											Remarks <span className="font-medium normal-case opacity-70">(Optional)</span>
										</label>
										<input
											type="text"
			...
			<div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[480px] p-4 bg-white border-t border-gray-200 z-40 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
