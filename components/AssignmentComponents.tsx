...
export function AssignmentSelect({ className = "", containerClassName = "", children, disabled, ...props }: SelectProps) {
	return (
		<div className={`relative ${containerClassName}`}>
			<select
				disabled={disabled}
				className={`w-full bg-gray-50 border border-gray-200 !rounded-lg !py-3 !pl-3 !pr-10 !text-xs appearance-none focus:ring-primary focus:border-primary truncate ${
...
export function AssignmentInput({ className = "", disabled, hasError = false, ...props }: InputProps) {
	return (
		<input
			disabled={disabled}
			className={`w-full bg-gray-50 border !rounded-lg !py-3 !px-3 !text-xs focus:ring-0 focus:border-gray-200 transiton-colors ${
